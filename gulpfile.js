...
gulp.task('js-inject', ['js'], function () {
    ...
    var sources = gulp.src([paths.dist.js + '/' + names.destApp + '*.js'], {read: false});
    return target.pipe(inject(sources))
        .pipe(gulp.dest(paths.dist.path));
});

gulp.task('style', ['cssClean'], function () {
...
gulp.task('style-inject', ['style'], function () {
    var target = gulp.src(paths.index);
    var sources = gulp.src([paths.dist.css + '/' + names.destApp + '*.css'], {read: false});
    return target.pipe(inject(sources))
        .pipe(gulp.dest(paths.dist.path));
});

gulp.task('watchStyle', function () {
    gulp.watch(paths.less, ['style-inject']);
});

gulp.task('watchJs', function () {
    gulp.watch(paths.js, ['js-inject']);
});
...
gulp.task('build', ['js-inject', 'style-inject']);
